...
private:
    ...
    static void ObserveAllPhysicsEvents(IWorld* world)
    {
        ...
        world->GetPhysicsEvents().soldier_throws_active_weapon.AddObserver(
          [world](const Soldier& soldier) {
              if (soldier.weapons[soldier.active_weapon].GetWeaponParameters().kind ==
                  WeaponType::Knife) {
                  spdlog::debug("soldier {} throws knife", soldier.id);
                  Weapon weapon{ WeaponParametersFactory::GetParameters(WeaponType::ThrownKnife,
                                                                        false) };
                  auto aim_x = (float)soldier.control.mouse_aim_x;
                  auto aim_y = (float)soldier.control.mouse_aim_y;
                  auto dir =
                    Calc::Vec2Normalize(glm::vec2(aim_x, aim_y) - soldier.skeleton->GetPos(15));
                  ...
                  auto inherited_vel = soldier.particle.GetVelocity() *
                                       weapon.GetWeaponParameters().inherited_velocity;
                  ...
                  world->GetStateManager()->ChangeSoldierPrimaryWeapon(soldier.id,
                                                                       WeaponType::NoWeapon);
              } else {
                  ...
                  glm::vec2 aim_direction = { soldier.control.mouse_aim_x,
                  ...
                  new_item.skeleton->SetPos(1,
                                            new_item.skeleton->GetPos(1) + weapon_throw_velocity);
                  weapon_throw_velocity = Calc::Vec2Scale(aim_direction, weapon_throw_speed_pos2);
                  new_item.skeleton->SetPos(2,
                                            new_item.skeleton->GetPos(2) + weapon_throw_velocity);
              }
          });
        world->GetPhysicsEvents().soldier_fires_primary_weapon.AddObserver(
          [world](Soldier& soldier) {
