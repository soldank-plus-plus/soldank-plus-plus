...
public:
                 ...
                 std::shared_ptr<IFileWriter> file_writer = std::make_shared<FileWriter>())
    {
        ...
        auto error =
          file_writer->Write(map_path, std::ios::out | std::ios::binary | std::ios::trunc);
    ...
    bool CollisionTest(glm::vec2 pos, glm::vec2& perp_vec, bool is_flag = false) const
    {
        ...
        if ((rx > 0) && (rx < GetSectorsCount() + 25) && (ry > 0) &&
            (ry < GetSectorsCount() + 25)) {
    ...
    void AddPolygons(const std::vector<PMSPolygon>& polygons)
    {
        ...
        while (old_polygons_id < old_polygons.size() ||
               polygons_to_add_id < polygons_to_add.size()) {
    ...
    void RemovePolygonsById(const std::vector<unsigned int>& polygon_ids)
    {
        ...
        while (polygon_id + removal_id < map_data_.polygons.size()) {
            ...
            if (removal_id > 0) {
                std::swap(map_data_.polygons[polygon_id],
                          map_data_.polygons[polygon_id + removal_id]);
            }
        ...
        polygon_vertices_with_new_color)
    {
        for (const auto& [polygon_vertex_id, new_color] : polygon_vertices_with_new_color) {
            map_data_.polygons.at(polygon_vertex_id.first)
              .vertices.at(polygon_vertex_id.second)
              .color = new_color;
        }
    ...
    void AddSpawnPoints(const std::vector<std::pair<unsigned int, PMSSpawnPoint>>& spawn_points)
    {
        ...
        while (old_spawn_point_id < old_spawn_points.size() ||
            ...
            if (spawn_points_to_add_id < spawn_points_to_add.size() &&
                spawn_points_to_add.at(spawn_points_to_add_id).first ==
                  map_data_.spawn_points.size()) {

                map_data_.spawn_points.push_back(
                  spawn_points_to_add.at(spawn_points_to_add_id).second);
    ...
    void SetSpawnPointsById(const std::vector<std::pair<unsigned int, PMSSpawnPoint>>& spawn_points)
    {
    ...
    unsigned int AddNewScenery(const PMSScenery& scenery, const std::string& file_name)
    {
        ...
        map_change_events_.added_new_scenery.Notify(map_data_.scenery_instances.back(),
                                                    new_scenery_id);
    ...
    PMSScenery RemoveSceneryById(unsigned int id)
    {
        ...
        if (!other_scenery_with_same_style_exists) {
            PMSSceneryType removed_scenery_type =
              map_data_.scenery_types.at(removed_scenery_style - 1);
            ...
            map_change_events_.removed_scenery.Notify(
              removed_scenery, id, map_data_.scenery_instances);
      ...
      const std::vector<std::pair<unsigned int, std::pair<PMSScenery, std::string>>>& sceneries)
    {
        ...
        std::sort(sceneries_to_add.begin(),
                  sceneries_to_add.end(),
        ...
        while (old_scenery_id < old_sceneries.size() ||
               sceneries_to_add_id < sceneries_to_add.size()) {

            if (sceneries_to_add_id < sceneries_to_add.size() &&
                sceneries_to_add.at(sceneries_to_add_id).first ==
                  map_data_.scenery_instances.size()) {
    ...
    static std::array<glm::vec2, 4> GetSceneryVertexPositions(const PMSScenery& scenery)
    {
        ...
        transform_matrix =
          glm::rotate(transform_matrix, -scenery.rotation, glm::vec3(0.0, 0.0, 1.0));
        transform_matrix =
...
private:
    ...
    void ReadPolygonsFromBuffer(std::stringstream& buffer)
    {
        ...
        for (int i = 0; i < polygons_count; i++) {
            ...
            new_polygon.bounciness = glm::length(
              glm::vec2(new_polygon.perpendiculars[2].x, new_polygon.perpendiculars[2].y));

            for (unsigned int j = 0; j < 3; j++) {
                glm::vec2 normalized_perpendiculars = Calc::Vec2Normalize(glm::vec2(
                  new_polygon.perpendiculars.at(j).x, new_polygon.perpendiculars.at(j).y));
    ...
    void ReadSceneryTypesFromBuffer(std::stringstream& buffer)
    {
        ...
        for (int i = 0; i < scenery_types_count; i++) {
            ...
            ReadStringFromBuffer(
              buffer, map_data_.scenery_types.back().name, SCENERY_NAME_MAX_LENGTH);
