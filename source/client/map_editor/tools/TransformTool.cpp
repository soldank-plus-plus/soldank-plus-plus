        ...
        switch (transform_mode_) {
            case TransformMode::Move: {
                ...
                for (const auto& selected_scenery_id :
                    ...
                    glm::vec2 position = { game_state_manager.GetConstMap()
                                             .GetSceneryInstances()
                ...
                for (const auto& selected_spawn_point_id :
                    ...
                    glm::ivec2 position = { game_state_manager.GetConstMap()
                                              .GetSpawnPoints()
                                              .at(selected_spawn_point_id)
                                              .x,
                                            game_state_manager.GetConstMap()
                                              .GetSpawnPoints()
                                              .at(selected_spawn_point_id)
                ...
                for (const auto& selected_soldier_id :
                    ...
                    game_state_manager.ForSoldier(selected_soldier_id, [&](const auto& soldier) {
                        soldier_positions.emplace_back(selected_soldier_id,
                                                       soldier.particle.position);
                    });
                }
            ...
            case TransformMode::Scale: {
                ...
                for (const auto& selected_scenery_id :
                    ...
                    sceneries.emplace_back(
                      selected_scenery_id,
                      game_state_manager.GetConstMap().GetSceneryInstances().at(
                        selected_scenery_id));
                }
                ...
                for (const auto& selected_spawn_point_id :
                     client_state.map_editor_state.selected_spawn_point_ids) {
                    spawn_points.emplace_back(selected_spawn_point_id,
                                              game_state_manager.GetConstMap().GetSpawnPoints().at(
...
private:
    static void SetupSelectionBox(ClientState& client_state, const StateManager& game_state_manager)
    {
        ...
        if (is_anything_selected) {
            ...
            client_state.map_editor_state.vertex_selection_box = { { left, top },
                                                                   { right, bottom } };
        } else {
            client_state.map_editor_state.vertex_selection_box = std::nullopt;
        }
    ...
    TransformMode transform_mode_{ TransformMode::Move };
};
} // namespace Soldank
